...
export class FuncionarioService {
  ...
  private apiUri = this.server + 'funcionario'

  constructor(private http : HttpClient) {}

   listar(){
      return this.http.get(this.server + 'funcionario').toPromise()
    }
    excluir(id: string){
        return this.http.request('DELETE',this.server + 'funcionario',{body:{_id:id}}).toPromise()
    }
    novo(body : any){
