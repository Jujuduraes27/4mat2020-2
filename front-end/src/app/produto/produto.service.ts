...
export class ProdutoService {
  ...
  private apiUri = this.server + 'produto'

  constructor(private http : HttpClient) {}

   listar(){
      return this.http.get(this.server + 'produto').toPromise()
    }
    excluir(id: string){
        return this.http.request('DELETE',this.server + 'produto',{body:{_id:id}}).toPromise()
    }
    novo(body : any){
